...
return [
    ...
    (new Extend\Settings)
        ...
        ->default('ekumanov-inline-audio.stripUploadPrefix', true)
        ->serializeToForum('ekumanov-inline-audio.stripUploadPrefix', 'ekumanov-inline-audio.stripUploadPrefix', 'boolval'),

    (new Extend\Formatter)
        ->configure(function ($config) {
            $config->BBCodes->addCustom(
                '[player title={TEXT;optional} start={REGEXP=/^\d+(:\d\d)?(:\d\d)?$/;optional}]{URL}[/player]',
                '<span class="pc-audio-wrap" data-audio-url="{URL}" data-start="{@start}" data-title="{@title}"></span>'
            );
        }),
